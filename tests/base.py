...
from crazy_workers.testing import polling
...
class BaseTestCase(unittest.TestCase):
  ...
  # Thin instance-method wrappers over the public crazy_workers.testing.polling
  # functions, kept for backwards compatibility of existing test cases.
  # ---------------------------------------------------------------------------

  def wait_for(self, condition, timeout=10.0, interval=0.05, msg='Condition never met'):
    polling.wait_for(condition, timeout=timeout, interval=interval, msg=msg)

  def wait_for_file(self, path, timeout=10.0):
    polling.wait_for_file(path, timeout=timeout)

  def wait_for_log(self, log_path, text, timeout=10.0):
    polling.wait_for_log(log_path, text, timeout=timeout)

  def wait_for_worker_status(self, manager, key, status, timeout=10.0):
    polling.wait_for_worker_status(manager, key, status, timeout=timeout)

  def wait_for_worker_in_db(self, manager, key, timeout=10.0):
    polling.wait_for_worker_in_db(manager, key, timeout=timeout)

  def wait_for_worker_pid(self, manager, key, timeout=10.0):
    polling.wait_for_worker_pid(manager, key, timeout=timeout)

  def wait_for_pid_dead(self, pid, timeout=10.0):
    polling.wait_for_pid_dead(pid, timeout=timeout)
